...
else if ($_POST['id'] == 'members_withnames')
{
    $where = "WHERE 1";
    if (!empty($_POST['q'])) {
        $where .= " AND `last_name` LIKE '%" . $this->mysql_cleans($_POST['q']) . "%'";
    }
    $q1 = $db->run_query("
        SELECT `member_id`,CONCAT(`first_name`,' ',`last_name`) as `display_name`
    ...
    ");
    while ($row = $q1->fetch()) {
        $list .= '<li onclick="return get_list_populate(\'' . $row['member_id'] . '\',\'' . $row['display_name'] . '\',\'' . $_POST['pop_id'] . '\',\'' . $_POST['pop_name'] . '\');">' . $row['display_name'] . '</li>';
    }
}
