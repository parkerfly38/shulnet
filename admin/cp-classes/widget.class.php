    ...
    function render_menu()
    {
        ...
        if ($cache['error'] != '1' && $this->recache != '1') {
            ...
            */
            ...
            if (!empty($this->theme['classes']))
            {
                $class = $this->theme['classes']['nav_ul'];
                $submenuClass = $this->theme['classes']['nav_item_dropdown'] ?? "zen_hoverable";
                $navItemclass = $this->theme['classes']['nav_item'] ?? "";
            }
            ...
            while ($item = $STH->fetch()) {
                ...
                if (! empty($submenu)) {
                    $this->final_content .= ' class="'.$submenuClass.'"  data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" ';
                }
                $this->final_content .= ' id="menu_item_' . $this->widget['id'] . '_' . $item['id'] . '">';
                if (! empty($submenu))
                {
                    $this->final_content .= '<div class="dropdown-menu" aria-labelledby="dropdown_'.$item['id'].'">';
                }
                $this->final_content .= $this->build_link($item);
                $this->final_content .= $submenu;
                if (!empty($submenu))
                {
                    $this->final_content .= '</div>';
                }
                $this->final_content .= '</li>';
            }
...
EOF;
    ...
    function build_link($item)
    {
        ...
        if (!empty($this->theme["classes"]))
        {
            $link .= ' class="'.$this->theme["classes"]["nav_link"] .'"';
        }
        $link .= '>';
        $link .= $item['title'];
        $link .= '</a>';
