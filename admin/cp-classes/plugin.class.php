...
class plugin extends db
{
    ...
    protected function get_config()
    {
        try {
            ...
            $this->add_history('error', '2', '', '4', '', 'ShulNET could not load the plugin config for ' . $this->id);
        }
    }
    ...
    public function getExtension()
    {
        try {
            ...
            $this->add_history('error', '2', '', '4', '', 'ShulNET could not find the ExtensionObject for plugin ' . $this->id);
        }
    }
    ...
    public function connectLocal()
    {
        if (! empty($this->global)) {
            try {
                ...
                $this->add_history('error', '2', '', '4', '', 'ShulNET could not connect to the database for plugin ' . $this->id);
            }
        } else {
    ...
    /**
     * Connects to the ShulNET database.
     */
