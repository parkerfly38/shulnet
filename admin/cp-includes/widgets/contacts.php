...
if ($options['list'] == '1') {
    ...
    while ($row = $STH->fetch()) {
        ...
        foreach ($options['fields'] as $header) {
            $cur++;
            if ($cur == 1) {
                $this_entry .= '<td><a href="returnull.php" onclick="return popup(\'contact_view\',\'id=' . $row['id'] . '\');">' . $sf->process($header, $row[$header]) . '</a></td>';
            } else {
...
<div class="nontable_section">
    <div class="pad24">
        <div class="widget_full"><a href="index.php?l=contacts">Full List &raquo;</a></div>
